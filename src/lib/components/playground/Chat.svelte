<script lang="ts">
	...
	type ChatImage = string | { url?: string; image_url?: { url?: string } };
	type ChatChoice = {
		delta?: { content?: string | null; images?: ChatImage[] };
		message?: { content?: string | null; images?: ChatImage[] };
	};
	...
	const chatCompletionHandler = async () => {
		...
		const applyChoice = (choice: ChatChoice) => {
			...
			if (images?.length) {
				responseMessage.images = [...(responseMessage.images ?? []), ...images];
			}
		};
